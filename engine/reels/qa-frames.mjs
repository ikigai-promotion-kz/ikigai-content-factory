 ...
 * заплачено подпиской. Отсюда и разница с `reels/qa.mjs`: тот шлёт кадры в
 * Claude API (≈$0,13 за 24-секундный ролик), этот стоит ноль.
 *
 * Когда какой брать:
 *   qa-frames.mjs — обычная приёмка, разбор с человеком, работа студента;
 *   qa.mjs        — когда вердикт нужен машинно: пакетный прогон, автоматика, CI.
 *
...
console.log('  2. Субтитры выше нижних 250 px — там интерфейс площадки. Сверху 120 px под ник.');
