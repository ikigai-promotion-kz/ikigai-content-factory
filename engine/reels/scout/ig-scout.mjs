...
//
// Две правки после боевого прогона 02.08.2026:
//  · подставляем ССЫЛКУ ПЕРВОЙ находки вместо плейсхолдера <ссылка> — иначе команду
//    нельзя скопировать, надо руками искать URL в выдаче выше;
//  · `--merge-output-format mp4` обязателен: без него yt-dlp склеивает дорожки в
//    webm и кладёт файл как `donor.mp4.webm` — дальше по цепочке имя не совпадает.
const top = filtered[0]?.url;
console.log('\nДальше — две команды. Скачать донора:');
console.log(`  yt-dlp -f "bv*[height<=1080]+ba/b" --merge-output-format mp4 -o donor.mp4 ${top || '<ссылка>'}`);
