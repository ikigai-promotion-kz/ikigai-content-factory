...
/**
 * Гейт промпта БОРДА. Заведён 15.08.2026, до этого борд не проверялся ничем.
 *
...
export function preflight(params, expect = {}) {
  ...
  const GUARDS = [
    ...
    // Четвёртая, 15.08.2026: требование воспроизвести подпись дословно. До неё правило
    ...
    // Пятая, 15.08.2026: набор титра одной гарнитурой. Дефект живёт не в словах, а в
