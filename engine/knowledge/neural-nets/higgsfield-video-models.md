...
|---|---|---|---|
| `seedance_2_0` | reference-driven лидер | `duration 4-15`; `resolution 480p/720p/1080p/4k`; `mode std/fast`; **`bitrate_mode standard/high`**; `genre`; `generate_audio`; **image/video/audio_references**; 7 AR (вкл. 21:9); start+end | до **12 референсов за проход** (9 img + 3 video ≤15с + 3 audio ≤15с + текст, модель сама читает роль каждого); аудио+видео одним проходом без пост-синка; frame-level консистентность лиц/одежды/стиля; лидер по aesthetics/physics/cinematography в тестах Higgsfield. **Лучший кандидат для карусель→видео с синхронным войсовером** |
...
**Три ручки Seedance, проверенные живым MCP 29.07.2026 — раньше стояли под вопросом:**
- **`bitrate_mode`** (`standard` | `high`) — параметр существует, точное имя именно такое (в срезе 11.07 он был записан как `bitrate`). Практика: `high` против полос (banding) на дыме, бликах и градиентах. Есть и у `seedance_2_0_mini`.
- **`mode` жёстко связан с `resolution`:** 4K и 1080p доступны **только** при `mode='std'`; `mode='fast'` умеет 480p/720p и ничего больше. Просить 4K на fast — ошибка вызова, а не «дешёвый 4K».
- **21:9 — это параметр `aspect_ratio`, а не текст промпта.** В разобранных роликах академии автор дописывал «Aspect 21:9» в конец промпта, тогда как UI-тумблер показывал 16:9. Спор снят: AR — контрол вызова. Правило движка неизменно: **меняет то, что видно в кадре → в промпт; меняет то, как результат произведён → в параметры.**

⚠ **Тарифный нюанс (не из seed):** на higgsfield.ai полный Seedance 2.0 недоступен на Starter/Basic (там только «Seedance 2.0 Fast»), полный доступ — с Plus/Pro+. Если у вас Starter или Basic — полного Seedance не будет; тариф проверяйте первым делом.
...
- **HappyHorse (Alibaba)** — вышел в топ Video Arena (апрель 2026, joint audio+video, 7-язычный lip-sync). **Интеграция ПОДТВЕРЖДЕНА живым MCP 29.07.2026:** id `happy_horse_video`, t2v + анимация одного start-frame, `resolution 720p/1080p`, `duration 3–15 с`, 5 AR (16:9, 9:16, 1:1, 4:3, 3:4). Единственная модель во всём каталоге, которой не было в срезе 11.07. Боем не проверялась — в роутер вносить после теста.
...
- Cinema Studio: **версии 3.5 в API не существует** (сверено живым MCP 29.07.2026). Есть ровно три id: `cinematic_studio_2_5` (image, до 4K), `cinematic_studio_video` и `cinematic_studio_video_v2` (видео, 3–12 с), плюс `cinematic_studio_3_0` (видео, 480p/720p/1080p/4k, `genre`, `generate_audio`, 4–15 с). Заявленные «3.5 / до 30 с» — маркетинг UI или будущий кап; API лимитирован 15 с. Практическое следствие: **цифра «Cinema Studio 3.5 = 9 680 кредитов» из академии относится к несуществующей в API модели** — в сметы не брать ни при каких условиях.
